    ...
    <div className="mb-5 min-w-0 overflow-x-auto border-b border-[#dfe8df]">
      <div className="flex min-w-max items-center gap-1" role="tablist" aria-label="Тайпалар">
        <button
          ...
          className={`border-b-2 px-4 py-3 text-sm font-extrabold transition ${activeGroupId === "all" ? "border-orange-500 bg-orange-50 text-orange-600" : "border-transparent text-muted hover:bg-slate-50"}`}
        ...
        {groups.map((group) => (
          <div className="flex items-center" key={group.id}>
            <button
              ...
              className={`border-b-2 px-4 py-3 text-sm font-extrabold transition ${activeGroupId === String(group.id) ? "border-orange-500 bg-orange-50 text-orange-600" : "border-transparent text-muted hover:bg-slate-50"}`}
        ...
        {onAddGroup && (
          <button className="inline-flex items-center gap-1.5 border-b-2 border-transparent px-4 py-3 text-sm font-extrabold text-orange-600 transition hover:bg-orange-50" onClick={onAddGroup} type="button">
