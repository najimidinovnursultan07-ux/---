  ...
  return (
    <section className="mb-5 rounded-lg border border-[#dfe8df] bg-white p-5 shadow-sm" id="group-management-panel">
      ...
      {canManage && <form className="mb-4 flex flex-wrap gap-2" onSubmit={handleSubmit}>
        <input className="h-10 min-w-44 flex-1 rounded-md border border-[#d6dfd8] px-3 text-sm outline-none focus:border-forest" onChange={(event) => setName(event.target.value)} placeholder="Тайпанын аты" required value={name} />
        {role === "ADMIN" && <select className="h-10 min-w-44 rounded-md border border-[#d6dfd8] bg-white px-3 text-sm outline-none focus:border-forest" onChange={(event) => setMentorId(event.target.value)} value={mentorId}><option value="">Менторду тандаңыз</option>{mentors.map((mentor) => <option key={mentor.id} value={mentor.id}>{mentor.full_name || mentor.email}</option>)}</select>}
        <button className="inline-flex h-10 items-center gap-2 rounded-md bg-ink px-4 text-xs font-extrabold text-white hover:bg-[#2b3a40]" type="submit"><Plus size={16} />Кошуу</button>
      </form>}
